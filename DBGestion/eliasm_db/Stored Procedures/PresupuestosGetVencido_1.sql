CREATE PROCEDURE [eliasm_db].[PresupuestosGetVencido]
	@Id varchar(150) = ''
AS
BEGIN
	SELECT P.Id,
			...
			ISNULL((SELECT SUM(Cantidad * d.cantidad) FROM PresupuestosDetalle D Where D.PresupuestoId = P.Id),0)CantidadProductos
	FROM Presupuestos P
