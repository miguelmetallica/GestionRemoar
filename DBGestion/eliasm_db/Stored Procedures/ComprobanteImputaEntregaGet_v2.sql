CREATE PROCEDURE [eliasm_db].[ComprobanteImputaEntregaGet_v2]
...
BEGIN TRY
	...
	BEGIN
		...
		SELECT P.Id ComprobanteId, D.Id DetalleId,D.ProductoId,D.Cantidad,ISNULL(CONVERT(NUMERIC(18,2),D.PrecioUnitario - (PrecioUnitario * (P.DescuentoPorcentaje / 100))),0)
		FROM Comprobantes P
		...
		BEGIN TRAN
			...
			BEGIN
				...
				WHILE @Cant > 0
				BEGIN
					INSERT INTO ComprobantesDetalleImputacion(Id,ComprobanteId,DetalleId,ProductoId,Cantidad,Precio,ImporteImputado,
															PorcentajeImputado,Estado,Fecha,Usuario,
															Entrega,FechaEntrega,UsuarioEntrega,
															AutorizaEntrega,FechaAutoriza,UsuarioAutoriza,
															Despacha,FechaDespacha,UsuarioDespacha,
															Devolucion,FechaDevolucion,UsuarioDevolucion,MotivoDevolucion)
					SELECT NEWID(),ComprobanteId,DetalleId,ProductoId,1,Precio,0 ImporteImputado,
							0 PorcentajeImputado,1 Estado,DATEADD(HH,4,GETDATE()) Fecha,NULL Usuario,
							0 Entrega,null FechaEntrega,null UsuarioEntrega,
							0 AutorizaEntrega,null FechaAutoriza,null UsuarioAutoriza,
							0 Despacha,null FechaDespacha,null UsuarioDespacha,
							0 Devolucion,null FechaDevolucion,null UsuarioDevolucion,null MotivoDevolucion
					FROM @COMP
					WHERE Num = @I
					SET @Cant = @Cant - 1
				END
