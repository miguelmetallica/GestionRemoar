CREATE PROCEDURE [eliasm_db].[VentasRapidasResumenGet]
	@Id varchar(150) = ''
AS
BEGIN
	...
	(SUM(P.TotalAPagar) - SUM(P.TotalAPagar) * (@InteresFormaPago / 100)) - sum(SaldoContado)
	...
	FROM(
		...
		0 DescuentoMonto,
		0 TotalAPagar,
		- SUM(D.importe) SaldoAPagar,
	    SUM(D.total) SaldoContado
		FROM VentasRapidas P
		INNER JOIN VentasRapidasFormasPagos D 
		ON D.VentaRapidaId = P.Id
		WHERE P.Id = @Id
		AND D.Importe <> 0
		AND D.Cuota <> 0
