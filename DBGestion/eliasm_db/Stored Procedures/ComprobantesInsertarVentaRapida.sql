CREATE PROCEDURE [eliasm_db].[ComprobantesInsertarVentaRapida]	
...
BEGIN TRY
	...
	DECLARE @TipoComprobanteEsDebe bit;
	...
	--DECLARE @TotalLista numeric(18,2);
	DECLARE @TotalSinImpuesto numeric(18,2);
	--DECLARE @TotalListaSinImpuesto numeric(18,2);
	DECLARE @TotalSinDescuento numeric(18,2);
	--DECLARE @TotalListaSinDescuento numeric(18,2);
	DECLARE @TotalSinImpuestoSinDescuento numeric(18,2);
	--DECLARE @TotalListaSinImpuestoSinDescuento numeric(18,2);
	DECLARE @DescuentoPorcentaje numeric(18,2);
	DECLARE @DescuentoTotal numeric(18,2);
	--DECLARE @DescuentoListaTotal numeric(18,2);
	DECLARE @DescuentoSinImpuesto numeric(18,2);
	--DECLARE @DescuentoListaSinImpuesto numeric(18,2);
	...
	SELECT @TipoComprobanteId = T.Id,
			...
			@TipoComprobanteEsDebe = T.EsDebe,
			@Letra = N.Letra,
	...
	WHERE C.Configuracion = 'COMPROBANTES_VENTA_RAPIDA'
	...
	SELECT 
		...
		@Telefono = P.Telefono,
		@Celular = P.Celular,
		@DescuentoPorcentaje = isnull(P.DescuentoPorcentaje,0),
	...
	SELECT 
	@Total = SUM((D.Precio * D.Cantidad) - ((D.Precio * D.Cantidad) * (ISNULL(CASE WHEN D.AceptaDescuento = 1 THEN P.DescuentoPorcentaje ELSE 0 END,0))/100)),
	@TotalSinImpuesto = SUM((D.PrecioSinImpuesto * D.Cantidad) - ((D.PrecioSinImpuesto * D.Cantidad) * (ISNULL(CASE WHEN D.AceptaDescuento = 1 THEN P.DescuentoPorcentaje ELSE 0 END,0))/100)) ,
	@TotalSinDescuento = SUM(D.Precio * D.Cantidad),
	@TotalSinImpuestoSinDescuento = SUM(D.PrecioSinImpuesto * D.Cantidad),
	@DescuentoTotal = SUM((D.Precio * D.Cantidad) * (ISNULL(CASE WHEN D.AceptaDescuento = 1 THEN P.DescuentoPorcentaje ELSE 0 END,0))/100),
	@DescuentoSinImpuesto = SUM((D.PrecioSinImpuesto * D.Cantidad) * (ISNULL(CASE WHEN D.AceptaDescuento = 1 THEN P.DescuentoPorcentaje ELSE 0 END,0))/100)
	FROM VentasRapidas P
	...
	BEGIN TRAN		
			...
			SELECT NEWID() Id,
				...
				D.Cantidad,
				P.UnidadMedidaId,
				U.Codigo UnidadMedidaCodigo,
				U.Descripcion UnidadMedida,
				P.CuentaVentaId,
				CV.Codigo CuentaVentaCodigo,
				CV.Descripcion CuentaVenta,
				D.Precio Precio,
				D.PrecioSinImpuesto PrecioSinIva,
				D.Precio * D.Cantidad SubTotal,
				D.PrecioSinImpuesto * D.Cantidad SubTotalSinIva,
				@DescuentoPorcentaje PorcentajeBonificacion,
				D.Precio * @DescuentoPorcentaje / 100 ImporteBonificacionLista,
				P.AlicuotaId AlicuotaIvaId,
				A.Descripcion AlicuotaIva,
				A.Codigo AlicuotaIvaCodigo,
				A.Porcentaje AlicuotaProcentaje,
				0 ImporteNetoNoGravado,
				0 ImporteExento,
				0 ImporteNetoGravado,
				...
				D.AceptaDescuento,
				DATEADD(HH,4,GETDATE()) FechaAlta,
				@Usuario
			...
			SELECT Id,
					...
					FechaAlta,
					UsuarioAlta
			FROM VentasRapidasFormasPagos
			WHERE VentaRapidaId = @VentaRapidaId


			UPDATE VentasRapidas
			SET ComprobanteId = @Id
			WHERE Id = @VentaRapidaId
	COMMIT;
