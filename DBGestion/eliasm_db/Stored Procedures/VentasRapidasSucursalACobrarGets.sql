CREATE PROCEDURE [eliasm_db].[VentasRapidasSucursalACobrarGets]
...
BEGIN
	SELECT P.Id,
		...
		SUM((D.Precio * D.Cantidad) - ((D.Precio * D.Cantidad) * (ISNULL(CASE WHEN D.AceptaDescuento = 1 THEN P.DescuentoPorcentaje ELSE 0 END,0))/100)) Precio,
		P.UsuarioAlta,
		P.ComprobanteId
	FROM VentasRapidas P
	INNER JOIN VentasRapidasDetalle D ON D.VentaRapidaId = P.Id
