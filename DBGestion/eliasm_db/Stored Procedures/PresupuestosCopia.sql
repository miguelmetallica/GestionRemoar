CREATE PROCEDURE [eliasm_db].[PresupuestosCopia]
	@Id nvarchar(150),
	@OldId nvarchar(150),
...
BEGIN TRY
	...
	BEGIN TRAN
		...
		SELECT NEWID(),@Id,D.ProductoId,P.PrecioVenta,P.PrecioVenta - (P.PrecioVenta * (A.Porcentaje / 100)),
		D.Cantidad,UPPER(@Usuario),DATEADD(HH,4,GETDATE())
		FROM PresupuestosDetalle D
		INNER JOIN Productos P ON P.Id = D.ProductoId		
		INNER JOIN ParamAlicuotas A ON A.Id = P.AlicuotaId
		WHERE D.PresupuestoId = @OldId
