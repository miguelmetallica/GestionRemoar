...
(function(){
			...
			case "object":
				if (mixed_val == null)
					...
					for (var i=0; i<mixed_val.length; i++) {
						...
						if (ser)
							map.push(mendel_serialize(idxobj.idx) + ser);
					}                                      
					...
					for (var prop in mixed_val) {
						...
						if (ser)
							props.push(mendel_serialize(prop) + ser);
					}
					if (obj_to_array)
	...
	/*
	// Usage example:
	var error = mendel_form_validate(jQuery(form_selector), {		// -------- Options ---------
	...
	*/
		...
		if (obj!==null && (typeof(obj)=='object' || typeof(obj)=='array')) {
			for (var prop in obj) {
				...
				if (level<recursive && (typeof(obj[prop])=='object' || typeof(obj[prop])=='array') && obj[prop]!=obj)
					dispStr += addStr + prop + '=' + mendel_debug_object(obj[prop], recursive, showMethods, level+1);
				else {
					try {
						curStr = "" + obj[prop];
					} catch (e) {
	...
	window.dco===undefined && (window.dco = function(s,r) { console.log(mendel_debug_object(s,r)); });
	window.dal===undefined && (window.dal = function(s) { if (MENDEL_STORAGE['user_logged_in']) alert(s); });
	window.dao===undefined && (window.dao = function(s,r) { if (MENDEL_STORAGE['user_logged_in']) alert(mendel_debug_object(s,r)); });
	window.ddl===undefined && (window.ddl = function(s) { mendel_debug_log(s); });
	window.ddo===undefined && (window.ddo = function(s,r) { mendel_debug_log(mendel_debug_object(s,r)); });
...
jQuery(document).ready(function() {
	...
	function mendel_ready_actions() {
		...
		if (document.documentElement.className.indexOf(MENDEL_STORAGE['site_scheme'])==-1)
			document.documentElement.className += ' ' + MENDEL_STORAGE['site_scheme'];
		...
		jQuery('.menu_mobile').on('click', 'li a, li a .open_child_menu', function(e) {
			...
			if (!jQuery(this).hasClass('open_child_menu') && mendel_is_local_link($a.attr('href')))
		...
		jQuery('.nav-links-more a').on('click', function(e) {
			...
			if (panel.length == 0) {															// Load simple page content
				jQuery.get(location.href, {
					...
					mendel_loadmore_add_items(jQuery('.content .posts_container').eq(0),
			...
			function mendel_loadmore_add_items(container, items) {
				...
				if (page+1 >= max_page)
					...
					MENDEL_STORAGE['load_more_link_busy'] = false;
	...
	function mendel_stretch_sidemenu() {
		...
		var toc_items_height = jQuery(window).height() 
								- mendel_fixed_rows_height(true, false) 
	...
	function mendel_check_layout() {
		...
		if (MENDEL_STORAGE['mobile_device'] || mendel_browser_is_mobile()) 
		...
		cont.find('.mendel-full-height').each(function () {
			...
			if (fullheight_row.length > 0)
				...
				var wh = screen.width >= 960
							? jQuery(window).height() - mendel_fixed_rows_height()
							: 'auto';
				if (wh > 0) {
	...
	function mendel_init_media_elements(cont) {
		if (MENDEL_STORAGE['use_mediaelements'] && cont.find('audio:not(.inited),video:not(.inited)').length > 0) {
            if (window.mejs) {
				...
				cont.find('audio:not(.inited),video:not(.inited)').each(function() {
					...
					if (jQuery(this).parents('.mejs-mediaelement').length == 0
						...
						&& jQuery( this ).parents( '.elementor-background-video-container' ).length === 0
							&& (MENDEL_STORAGE['init_all_mediaelements']
				...
				setTimeout(function() { mendel_init_media_elements(cont); }, 400);
		}
	}


	// Fit video frames to document width
	function mendel_resize_video(cont) {
		if (cont === undefined) {
			cont = $body;
		}
		cont.find( 'video' ).each(
			function() {
				...
				var video     = $self.addClass( 'mendel_resize' ).eq( 0 );
				...
				if ( percent ) {
					...
					if ( mejs_cont.length > 0 ) {
						mendel_set_mejs_player_dimensions( video, w_real, h_real );
					}
				}
		...
		cont.find( '.video_frame iframe,iframe' ).each(
			function() {
				var $self = jQuery( this );
				// If item now invisible
				if ( $self.hasClass( 'trx_addons_resize' ) || $self.hasClass( 'trx_addons_noresize' ) || $self.addClass( 'mendel_resize' ).parents( 'div:hidden,section:hidden,article:hidden' ).length > 0 ) {
					return;
				}
	...
	function mendel_set_mejs_player_dimensions(video, w, h) {
		if (mejs) {
	...
	function mendel_tabs_ajax_content_loader(panel, page, oldPanel) {
		if (panel.html().replace(/\s/g, '')=='') {
			...
			panel.find('> *').addClass('mendel_tab_content_remove');
	...
	function mendel_comments_validate(form) {
		...
		var comments_args = {
			error_message_text: MENDEL_STORAGE['strings']['error_global'],	// Global error message text (if don't write in checked field)
			error_message_show: true,									// Display or not error message
			error_message_time: 4000,									// Error message display time
			error_message_class: 'mendel_messagebox mendel_messagebox_style_error',	// Class appended to error message block
			error_fields_class: 'error_field',							// Class appended to error fields
			exit_after_first_error: false,								// Cancel validation and exit after first error
			rules: [
				{
					...
					max_length: { value: MENDEL_STORAGE['comment_maxlength'], message: MENDEL_STORAGE['strings']['text_long']}
				}
			]
		};
...
jQuery(document).on('action.init_hidden_elements', function(e, cont) {
	...
	if (MENDEL_STORAGE['button_hover'] && MENDEL_STORAGE['button_hover']!='default') {
		jQuery('button:not(.search_submit):not([class*="sc_button_hover_"]),\
				...
				.wp-block-button:not(.is-style-outline) .wp-block-button__link:not([class*="sc_button_hover_"]),\
				.sc_button:not([class*="sc_button_simple"]):not([class*="sc_button_hover_"]),\
				.sc_form_field button:not([class*="sc_button_hover_"]),\
				.post_item .more-link:not([class*="sc_button_hover_"]),\
				.trx_addons_hover_content .trx_addons_hover_links a:not([class*="sc_button_hover_"]),\
				.mendel_tabs .mendel_tabs_titles li a:not([class*="sc_button_hover_"]),\
				.hover_shop_buttons .icons a:not([class*="sc_button_hover_style_"]),\
				.mptt-navigation-tabs li a:not([class*="sc_button_hover_style_"]),\
				.edd_download_purchase_form .button,\
				.edd-submit.button,\
				#btn-buy:not([class*="shop_"]):not([class*="sc_button_hover_"]),\
				...
				').addClass('sc_button_hover_just_init sc_button_hover_'+MENDEL_STORAGE['button_hover']);
		...
		jQuery('.sidebar .trx_addons_tabs .trx_addons_tabs_titles li a:not([class*="sc_button_hover_style_"]),\
				.mendel_tabs .mendel_tabs_titles li a:not([class*="sc_button_hover_style_"]),\
		...
		jQuery('.mejs-controls button,\
				...
				').removeClass('sc_button_hover_'+MENDEL_STORAGE['button_hover']);
...
(function() {
	...
	jQuery(document).on('action.prepare_stretch_width', function() {
		if (MENDEL_STORAGE['stretch_tabs_area'] > 0)
